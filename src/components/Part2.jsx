...
///////////////////////////////////////////////////////////////////////////////  
 ...
 const tester = _.map(sortLodFruits,(sl, index)=><li key={index}>{sl}</li>) // put reverse() for descending
  const objs = [{ id: 1, name: 'Object 1', test: [1,2,3,[50],{Mani: "he is a potta"}] }, { id: 2, name: 'Object 2' }, { id: 3, name: 'Object 3' }];
  // lodash map
  const mapperLod = _.map(fruits,(f, index)=> <li key={index}>{f+' * '}</li>)
  const mapper = fruits.map((fruit, index) => <li key={index}>{fruit}</li>);
  const objmapperLod = _.map(objs,(obj, index)=> <li key={index}>{obj.id+' is'}-{obj.name}</li>)
  // lodash filter
  const filLodfruits = _.filter(fruits, (f) => f !== "mango" && f !== "apple");

  const nativeLodFilter = _.filter(people,(p)=>p.age<30)
  const filterLodvals = _.map(nativeLodFilter,(a,index)=><li key={index}>{a.name}</li>)
